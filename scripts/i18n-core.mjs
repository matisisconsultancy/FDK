...
/* The text fragments inside a block key — what the engine falls back to
   translating when the block as a whole has no entry. */
export function innerTexts(k) {
  if (!k.includes("<")) return [];
  const out = [];
  (function walk(n) {
    for (const c of n.children || []) {
      if (c.type === "text") { const t = normWS(c.value).trim(); if (/[A-Za-zÀ-ÿ]/.test(t)) out.push(t); }
      else walk(c);
    }
  })(parseHTML(k));
  return out;
...
/* Every string the engine may look up while translating this key: the key
   itself, the text inside its markup, and the segments of any "·" label. */
export function lookupParts(k) {
  const out = new Set();
  const add = (t) => {
